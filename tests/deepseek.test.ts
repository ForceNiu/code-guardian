...
// 失败与重试（真实实现最多 4 次尝试）
// 分类口径（2026-09-17 收紧后）：
//   · 4xx（429 除外）→ 确定性错误，**不重试**，只发 1 次请求
//   · 429           → 按限流退避重试（1000 / 2000 / 4000ms）
//   · 5xx / 网络异常 → 临时性故障，退避重试到上限 4 次（500 / 1000 / 2000ms）
// ---------------------------------------------------------------------------

test("400 是确定性错误 → 不重试，只发 1 次请求", async () => {
  resetNet();
  httpsHolder.responses = [{ status: 400, body: "invalid model" }];

  await assert.rejects(
    () => runWithFakeTimers(() => new DeepSeekLLM("k").invoke("hello")),
    /DeepSeek 返回 400/,
  );
  assert.equal(httpsHolder.calls.length, 1, "4xx 是确定性错误，不该进退避轮次");
});
