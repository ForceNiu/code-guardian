...
- 📌 **同源后续**：**U6 / U7 已于本会话落地**（见 §二.13 / §二.14 的 ✅ 标注）—— U6(a) 卡片标注「全仓依赖体检（与本次改动无关）」、U6(b/c) 新增 `detectChangedDeps` 比对 base/head 的 package.json 并给新增依赖打「新增」徽标；U7(a) 表头「影响范围」→「中招版本」并改写导语、U7(b) `isDev` 标记让 devDependencies 在漏洞表标「（dev）」。~~**U4(a)(b)**（覆盖范围声明 + 装饰器插件）仍**未做，仅规划**~~ → **2026-09-21 更正：已随后续 A 组落地**（A1 声明范围 / A2 加 `decorators-legacy` · PR #26 · 2026-09-20），见 §二.12 的 ✅ 标注。
...
#### 审计收口批（第三轮 · 逐文件 ＋ 结构级）· `AUD-19`–`AUD-28`（2026-09-20 深夜）

> **来源**：用户口令「再次进行最后的文档内容检查，需要非常仔细的进行逐个文件的内容梳理」。
> **换靶子**：前两轮按**问题类型**扫；本轮**按文件**逐个走，并**首次把「结构」也当内容核** ——
> 行号引用、计数、目录列举、配置三件套一致性。其中 `docs/DEVELOPING.md` 是**首次全文逐行**（前两轮只做关键字扫，一补就出 2 条 🔴）。
> **手段**：① 逐文件通读；② **机械穷举**：4 份手写文档共 9 处 `file:line`（**8 对 1 偏**，偏的即 `AUD-20`）＋ 本台账 102 处行号引用（**93 可定位**，其余 9 处全指向**仓库外已归档件** —— 属 `AUD-7` 判例的「**有意保留的历史路径**」，非死链）。
> **结论**：🔴 4 条 ＋ 🟡 3 条 ＋ ⚪ 3 条 **全部已改**。**其中 2 条 🔴 是「上一轮修改动作本身」制造的回归**（`AUD-19` / `AUD-20`），不是文档原有缺陷。
> **状态**：⏸️ **已改，未提交** —— 改动留在磁盘上等批准（协议⑤），经批准后走 分支 → PR → CI → squash merge。
> **本批的硬约束（已执行并自证）**：**16 增 / 16 删、净 0 行** —— 全部改动**不改变任何文件的行数**，从根上杜绝行号漂移；且凡引用位置一律**改写成标识符或命令**，不再写死行号。

| 新号 | 位置 | 写的 | 实际 | 处置 |
|---|---|---|---|---|
| **AUD-19** | `.env.example:10` ＋ `src/lib/ai/semantic-graph.ts:2` | 「规则引擎无法确定（**置信度 &lt;70%**）」 | 🔴 **「70%」是虚构数值**：`confidence` 是三档**枚举**（`proven` / `heuristic` / `uncertain`，见 `src/lib/types.ts:80`、`worker/rules.cjs:6`），**全仓没有任何数值置信度**。来历 `ab0d85e`（既有提交，非本轮引入）。<br>🔴 **且是第一轮 `AUD-1` 的漏网**：原句**同一行里有两个数**（`80%` ＋ `70%`），`AUD-1` 只删了 `80%`、**把同源的 `70%` 留在了原地** → 等于**用一个虚构数值换另一个虚构数值** | ✅ **已修**：两处删掉百分比，只留「判为 `uncertain` 的变更」。复测：`git grep '70%'` 应 **0 命中** |
| **AUD-20** | `docs/DEVELOPING.md:201` ＋ `src/lib/run-analysis.ts:23` | 同步点写作 `src/lib/run-analysis.ts:23` | 🔴 **行号被 `AUD-9` 顶偏 1 行**：`git show d7a0bf6` 对比 —— 改动前注释 1 行、`const workerPath` 在 **`:23`**（当时该引用**是对的**）；`AUD-9` 把那条注释从 1 行扩成 2 行 → 现在在 **`:24`**。连代码注释里特意写的「**本行**」也一起失效 | ✅ **已修（治本）**：**两处都不再写行号** —— 改写成「本文件的 `const workerPath`」＋附 `grep -rn 'analyze\.worker\.cjs'` 穷举命令。**这一类引用从此不会漂** |
| **AUD-21** | `docs/DEVELOPING.md:29` | 「只统计 `.ts` 会漏掉**两个**引擎测试文件」 | 实际 `tests/*.test.cjs` 有 **3 个**且全是引擎测试：`analyze-core` / `analyze-worker` / `rules` | ✅ **已修**为「**三个**」并列名。📌 正撞本文件 §3 附则自己立的规矩「说『从未 / 唯一 / 全部』前先穷举」 |
...
> `AUD-19` / `AUD-20` **都是「修改动作本身」制造的**，而不是文档原有缺陷。两个根因各对应一条硬规矩：
> 1. **`AUD-19`**：改的是「**这一行**」而不是「**这个说法**」→ **改任何说法前，先搜全仓该说法的所有出现处；要么全改，要么一处不改。**
> 2. **`AUD-20`**：**改行数后没重算引用** → **加 / 删行后必须重算该文件之后的全部 `file:line`**（本批即按此执行：**净 0 行**，从根本上不产生漂移）。
> **治本方向（本批已落地）**：**行号 / 计数这类「自指结构」不要写死** —— 写标识符（`const workerPath`）或写命令（`grep -rn …`）。这与本文件 §1 的「**写怎么量而非量出来多少**」是同一条纪律。
> **收口建议**：这 10 条落完并合并后，**「文档准确性体检」即可关闭** —— 之后只在「代码改动涉及文档」时顺带更新，**不再单独开审计轮次**。
...
|---|---|---|---|
| ~~**U4**~~ | ✅ **已修：(a)+(b) 已随 A 组落地（A1/A2 · PR #26 · 2026-09-20）；(c) 已由 D10 判为范围外**。~~**支持边界太窄 → 缺前端通用性**~~（下表为**修复前**的实测证据） | ① `SOURCE_EXT`（`worker/analyze-core.cjs:10`）只有 `.js` / `.jsx` / `.ts` / `.tsx` / `.mjs` / `.cjs` → 🔴 **`.vue` 根本不在名单**（连解析都不会尝试，纯 Vue 项目上基本无用）② **装饰器实测 FAIL**（报「需要开启 `decorators` / `decorators-legacy`」，本项目只开了 typescript + jsx）→ Angular / NestJS / TypeORM 风格项目**每个文件都进 `catch`**，影响链路整体失效 | **(a) 已做** —— README 已知边界表声明「仅 TS/JS，不含 .vue/.svelte」（A1）；**(b) 已做** —— `plugins` 加 `decorators-legacy`，解锁装饰器项目（A2，`worker/analyze-core.cjs`）；**(c) ⛔ 范围外** —— 支持 `.vue` 判为不做，理由见 D10（要拆 `<script>` 块 + 行号偏移映射 + 模板引用，成本量级不同） |
...
| webhook 无事件头兼容路径 | 属向后兼容分支（`src/app/api/webhook/route.ts` 中无事件头时走的那处 `safeEqual`） | 发不带 `X-GitHub-Event` / `X-Gitlab-Event` 的请求 |
