...
### 2026-09-17 第六批 · 全量文本正确性审计（工作区，**未提交**）
...
### 2026-09-17 第七批 · 全量结构审计重跑（工作区，**未提交**）
...
- 状态：**未开工**。范围摘要 = 功能 + 架构两层里的第一层（即先只做**功能层**），HTML 单页，全真口径。
- ⚠️ **原始范围定义已丢失**：原定开场白写在 `docs/HANDOVER.md` §8，而该文件是本地件、从未入库、现已删除（E3）
  → **那句开场白连同 G1 的详细范围一起失效**，全仓只剩上面这一句摘要。
- ✅ **2026-09-17 已重写为 `docs/G1-KICKOFF.md`**（自包含：项目背景 / 范围对齐 / 硬约束 / 学习取向 / 必读清单）。
  新对话直接说「读 `docs/G1-KICKOFF.md`，按它开工」。
  🔴 因此**开工第一步是「重新对齐范围」**（读者是谁 / 只做功能层还是连架构层 / 要不要交互），**不是动手写**。
