...
| 难点 | 方案 | 落地位置 |
| :--- | :--- | :--- |
| AST 遍历阻塞主线程 | `worker_threads` 隔离 CPU，主线程只做 IO | `worker/analyze.worker.cjs` |
...
> 定位：把「代码副作用」审查扩展到「依赖安全」维度，产出与影响链路并列的第二份门禁报告。已完成 CVE 扫描 + 构建体积检测 + GitLab 状态回写。
